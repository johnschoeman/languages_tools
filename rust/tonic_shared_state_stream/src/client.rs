...
async fn publish(client: &mut StreamClient<Channel>, value: u64) {
    client.publish(pb::PublishRequest { message: value.to_string() }).await.unwrap();
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    publish(&mut client, 42).await;
    tokio::time::sleep(Duration::from_secs(1)).await;

    publish(&mut client, 100).await;
    tokio::time::sleep(Duration::from_secs(1)).await;
